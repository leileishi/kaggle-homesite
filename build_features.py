...
def main():
    ...
    for i, df in enumerate(train_test_dfs):
        df.iloc[:train_test_split].to_csv(args.train_feature + str(i), index=False, header=True)
        df.iloc[train_test_split:].to_csv(args.test_feature + str(i), index=False, header=True)
        del df
        gc.collect()

    print('Merge feature files for training data')
    train_feature_files = [args.train_feature + str(i) for i, df in enumerate(train_test_dfs)]
    train_file_cmd = 'paste -d , ' + ' '.join(train_feature_files) + ' > ' + args.train_feature
    ...
    test_feature_files = [args.test_feature + str(i) for i, df in enumerate(train_test_dfs)]
    test_file_cmd = 'paste -d , ' + ' '.join(test_feature_files[:-1]) + ' > ' + args.test_feature
